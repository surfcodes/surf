...
import { replaceBrowserUrl } from 'vs/githubSurf/util';
	...
	constructor(
		...
		if (enabled && lifecycleService.startupKind !== StartupKind.ReloadedWindow) {
			...
			getCurrentAuthority(commandService).then((authority: string) => {
				...
				if (fileState.path !== '/' && (!activeResource || activeResource.scheme !== 'githubsurf' || activeResource.path !== fileState.path)) {
					const currentFileUri = URI.from({ scheme: 'githubsurf', authority, path: fileState.path });
...
function getCurrentAuthority(commandService: ICommandService): Promise<string> {
	return commandService.executeCommand('githubsurf.get-current-authority') as Promise<string>;
}
...
class WelcomePage extends Disposable {
	...
	getGitHubTokenStatus() {
		return this.commandService.executeCommand('githubsurf.validate-token', true);
	}
