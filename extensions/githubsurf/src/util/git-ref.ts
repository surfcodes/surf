...
const getBrowserUri = (): Promise<vscode.Uri> => {
	return (vscode.commands.executeCommand('githubsurf.vscode.get-browser-url') as Promise<string>).then(browserUrl => {
		const browserUri = vscode.Uri.parse(browserUrl);
		const [owner = 'bridgedxyz', repo = 'github.surf'] = browserUri.path.split('/').filter(Boolean);
		currentOwner = owner;
		currentRepo = repo;
		return browserUri;
	});
...
const getRepositoryBranchesFromUri = reuseable((uri: vscode.Uri, forceUpdate: boolean = false): Promise<RepositoryBranch[]> => {
	...
	const [owner = 'bridgedxyz', repo = 'github.surf'] = uri.path.split('/').filter(Boolean);
...
const getCurrentRefFromUri = reuseable((uri: vscode.Uri, forceUpdate: boolean = false): Promise<string> => {
	...
	// this url should looks like `https://github.com/bridgedxyz/github.surf/tree/master/src`
...
// get the github.surf Uri authority from current browser url
...
export const changeCurrentRef = (newRef: string): Promise<string> => {
	return getBrowserUri().then((uri: vscode.Uri) => {
		vscode.commands.executeCommand('githubsurf.vscode.replace-browser-url', updateRefInUri(uri, newRef).toString());
